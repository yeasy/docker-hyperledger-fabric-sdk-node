...
RUN apk add --no-cache bash git g++ make python3 vim

ARG FABRIC_SDK_NODE_REF=v2.2.20
RUN cd /opt \
        && git clone --single-branch -b "${FABRIC_SDK_NODE_REF}" --depth 1 https://github.com/hyperledger/fabric-sdk-node.git \
        && cd fabric-sdk-node \
        && npm install
